(() => {
    ...
    window.addEventListener("scroll", (function() {
        if (this.scrollY >= 50 && !stikyBlock.classList.contains("not-transparent")) stikyBlock.classList.add("not-transparent"); else if (this.scrollY < 50 && stikyBlock.classList.contains("not-transparent")) stikyBlock.classList.remove("not-transparent");
    }));
    ...
    document.querySelector(".main-block__translate");
    ...
    function ActivateBlock(ActiveBlock, BlockMassive) {
        ...
        if (document.querySelector("header").classList.contains("menu-open")) OpenMenu();
    }
    function DellClassAcive(DellActive) {
        if (DellActive.classList.contains("active")) DellActive.classList.remove("active");
    }
    ...
    function BlockIsVisible(target) {
        ...
        if (targetPosition.top <= windowPosition.middle && targetPosition.bottom >= windowPosition.middle && target != visibleBlock) {
            visibleBlock = target;
            target.classList.forEach((targetclass => {
                if (-1 != menuLinksDataScrols.indexOf(targetclass)) {
